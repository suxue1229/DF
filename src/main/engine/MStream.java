...
public class MStream {
    ...
    public void parT(double value) throws Exception {
        if (value < 3 || value > 45) {
            throw new Exception("输入温度不在有效范围内[3~45]。");
        }
        this.parT = value;
    }
    ...
    private double parmPO43() {
        return ion(EIon.PO4).parmj() * parKH3PO4() * parKH2PO4() * parKHPO4() /
                (Math.pow(Math.pow(10, -parpH), 3)
                        + Math.pow(Math.pow(10, -parpH), 2) * parKH3PO4()
                        + Math.pow(10, -parpH) * parKH3PO4() * parKH2PO4()
                        + parKH3PO4() * parKH2PO4() * parKHPO4());
    }
    ...
    public double parSCaCO3() {
        return parpH - (
                -Math.log10(ion(EIon.Ca).parmj())
                        - Math.log10(ion(EIon.HCO3).parmj())
                        - Math.log10(parKHCO3())
    ...
    public double parSCa3PO42() {
        return Math.pow(ion(EIon.Ca).parmj(), 3) * Math.pow(parmPO43(), 2) / parKCa3PO42();
    }
    ...
    public void codmode(boolean unique) {
        cods.clear();
        if (unique) {
            cods.add(new MCOD("有机物"));
        } else {
            cods.add(new MCOD("有机物1"));
            cods.add(new MCOD("有机物2"));
        }
    }
